...
@property(strong, nonatomic)
    NFCReaderSession *nfcSession API_AVAILABLE(ios(11.0));
@property(strong, nonatomic)
    NFCNDEFMessage *messageToWrite API_AVAILABLE(ios(11.0));
@end
...
- (void)writeTag:(CDVInvokedUrlCommand *)command API_AVAILABLE(ios(13.0)) {
  ...
  @try {
    for (id recordData in ndefData) {
      ...
      NSData *type =
          [self uint8ArrayToNSData:[recordData objectForKey:@"type"]];
      NSData *identifier =
          [self uint8ArrayToNSData:[recordData objectForKey:@"identifiers"]];
      NSData *payload =
          [self uint8ArrayToNSData:[recordData objectForKey:@"payload"]];
      NFCNDEFPayload *record = [[NFCNDEFPayload alloc] initWithFormat:tnf
                                                                 type:type
    ...
    CDVPluginResult *pluginResult =
        [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR
                          messageAsString:@"Invalid NDEF Message"];
  ...
  if (self.nfcSession && self.nfcSession.isReady) {  // reuse existing session
    ...
    if (self.shouldUseTagReaderSession) {
      ...
      NSLog(@"Using NFCTagReaderSession");
      self.nfcSession = [[NFCNDEFReaderSession new] initWithDelegate:self
                                                               queue:nil
...
- (void)invalidateSession:(CDVInvokedUrlCommand *)command {
  ...
  // Always return OK. Alternately could send status from the
...
- (void)readerSession:(NFCNDEFReaderSession *)session
  ...
  if (tags.count > 1) {
    session.alertMessage =
        @"More than 1 tag detected. Please remove all tags and try again.";
...
- (void)readerSession:(NFCNDEFReaderSession *)session
    didInvalidateWithError:(NSError *)error API_AVAILABLE(ios(11.0)) {
  NSLog(@"readerSession ended");
  if (error.code ==
      NFCReaderSessionInvalidationErrorFirstNDEFTagRead) {  // not an error
...
- (void)tagReaderSession:(NFCTagReaderSession *)session
  ...
  [session connectToTag:tag
      completionHandler:^(NSError *_Nullable error) {
        ...
        id<NFCISO15693Tag> NfcVTag = [tag asNFCISO15693Tag];
        [NfcVTag
            readSingleBlockWithRequestFlags:NFCISO15693RequestFlagHighDataRate
                                blockNumber:9
                          completionHandler:^(NSData *_Nonnull data,
                                              NSError *_Nullable error) {
                            if (error) {
                              NSLog(@"reading single block error %@", error);
                              [self closeSession:session
                                       withError:@"Error reading tag."];
                              return;
                            }
                            NSLog(@"data %@", data);
                            NSMutableDictionary *nfcData =
                                [NSMutableDictionary new];
                            tagMetaData[@"nfcReading"] = [self uint8ArrayFromNSData:data];
            nfcData[@"tag"] = [self buildTagDictionary:nil metaData:tagMetaData];
                            CDVPluginResult *pluginResult = [CDVPluginResult
                                   resultWithStatus:CDVCommandStatus_OK
                                messageAsDictionary:nfcData[@"tag"]];
...
- (void)startScanSession:(CDVInvokedUrlCommand *)command {
  ...
  if (@available(iOS 13.0, *)) {
    ...
    CDVPluginResult *pluginResult;
    pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR
                                     messageAsString:@"NFC requires iOS 11"];
    [self.commandDelegate sendPluginResult:pluginResult
                                callbackId:command.callbackId];
  }
}

- (void)processNDEFTag:(NFCReaderSession *)session
                   tag:(__kindof id<NFCNDEFTag>)tag API_AVAILABLE(ios(13.0)) {
  [self processNDEFTag:session tag:tag metaData:[NSMutableDictionary new]];
}

- (void)processNDEFTag:(NFCReaderSession *)session
                   tag:(__kindof id<NFCNDEFTag>)tag
              metaData:(NSMutableDictionary *_Nonnull)metaData
    API_AVAILABLE(ios(13.0)) {
  [tag
      queryNDEFStatusWithCompletionHandler:^(
          NFCNDEFStatus status, NSUInteger capacity, NSError *_Nullable error) {
        if (error) {
          NSLog(@"%@", error);
...
- (void)readNDEFTag:(NFCReaderSession *_Nonnull)session
             status:(NFCNDEFStatus)status
                tag:(id<NFCNDEFTag>)tag
           metaData:(NSMutableDictionary *_Nonnull)metaData
    API_AVAILABLE(ios(13.0)) {
  ...
  [tag readNDEFWithCompletionHandler:^(NFCNDEFMessage *_Nullable message,
                                       NSError *_Nullable error) {
    // Error Code=403 "NDEF tag does not contain any NDEF message" is not an
    // error for this plugin
...
- (void)writeNDEFTag:(NFCReaderSession *_Nonnull)session
              status:(NFCNDEFStatus)status
                 tag:(id<NFCNDEFTag>)tag API_AVAILABLE(ios(13.0)) {
  switch (status) {
    case NFCNDEFStatusNotSupported:
      [self closeSession:session
               withError:@"Tag is not NDEF compliant."];  // alternate message
                                                          // "Tag does not
                                                          // support NDEF."
      break;
    ...
    case NFCNDEFStatusReadWrite: {
      [tag writeNDEF:self.messageToWrite
          completionHandler:^(NSError *_Nullable error) {
            if (error) {
              NSLog(@"%@", error);
              ...
              CDVPluginResult *pluginResult =
                  [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
              [self.commandDelegate sendPluginResult:pluginResult
                                          callbackId:self->sessionCallbackId];
              [self closeSession:session];
            }
          }];
      break;
...
- (void)closeSession:(NFCReaderSession *)session
           withError:(NSString *)errorMessage API_AVAILABLE(ios(11.0)) {
...
// TODO rename method since we're using the channel or callback instead of
// firing an event
- (void)fireNdefEvent:(NFCNDEFMessage *)ndefMessage
             metaData:(NSDictionary *)metaData API_AVAILABLE(ios(11.0)) {
  NSLog(@"fireNdefEvent");
  ...
  if (sessionCallbackId && self.returnTagInCallback) {
    ...
    CDVPluginResult *pluginResult =
        [CDVPluginResult resultWithStatus:CDVCommandStatus_OK
                      messageAsDictionary:nfcEvent[@"tag"]];
...
- (NSDictionary *)buildTagDictionary:(NFCNDEFMessage *)ndefMessage
...
- (NSString *)dictionaryAsJSONString:(NSDictionary *)dict {
  ...
  if (!jsonData) {
    jsonString = [NSString
        stringWithFormat:@"Error creating JSON for NDEF Message: %@", error];
    NSLog(@"%@", jsonString);
  } else {
