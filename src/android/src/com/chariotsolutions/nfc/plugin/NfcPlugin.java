...
public class NfcPlugin extends CordovaPlugin implements NfcAdapter.OnNdefPushCompleteCallback {
    ...
    private Intent onLaunchIntent = null; // custom shit, save the intent on launch, turn to null once done
    ...
    private void connect(final String tech, final int timeout, final CallbackContext callbackContext) {
        this.cordova.getThreadPool().execute(() -> {
            try {
                ...
                if (tag == null) { // if it is null, restart nfc and try again
                    tag = onLaunchIntent.getParcelableExtra(NfcAdapter.EXTRA_TAG);
                    onLaunchIntent = null;
                }

                if (tag == null) { // if it is still null, fail
                    Log.e(TAG, "No Tag");
